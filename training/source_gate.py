...
def admit(record: dict) -> tuple[bool,str]:
    cls=str(record.get("memory_class","")).upper(); path=str(record.get("object_path","")).upper()
    validated=bool(record.get("validated",False)); promoted=bool(record.get("promoted",False))
